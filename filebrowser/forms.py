...
class CreateDirForm(forms.Form):
    ...
    def clean_name(self):
        ...
        if self.cleaned_data["name"]:
            ...
            if self.site.storage.isdir(
                os.path.join(self.path, convert_filename(self.cleaned_data["name"]))
            ):
...
class ChangeForm(forms.Form):
    ...
    def clean_name(self):
        ...
        if self.cleaned_data["name"]:
            ...
            if (
                self.site.storage.isdir(
                    os.path.join(self.path, convert_filename(self.cleaned_data["name"]))
                )
                and os.path.join(self.path, convert_filename(self.cleaned_data["name"]))
                != self.fileobject.path
            ):
                raise forms.ValidationError(_("The Folder already exists."))
            elif (
                self.site.storage.isfile(
                    os.path.join(self.path, convert_filename(self.cleaned_data["name"]))
                )
                and os.path.join(self.path, convert_filename(self.cleaned_data["name"]))
                != self.fileobject.path
