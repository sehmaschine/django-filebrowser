    ...
    for fileobject in fileobjects:
        ...
        try:
            new_image.save(
                ...
                optimize=(os.path.splitext(fileobject.path)[1].lower() != ".gif"),
            )
        except IOError:
            new_image.save(
                tmpfile, format=Image.EXTENSION[ext], quality=VERSION_QUALITY
            )
        ...
        try:
            ...
            if saved_under != fileobject.path:
                fileobject.site.storage.move(
                    saved_under, fileobject.path, allow_overwrite=True
                )
        ...
        messages.add_message(
            request,
            messages.SUCCESS,
            _("Action applied successfully to '%s'" % (fileobject.filename)),
        )
