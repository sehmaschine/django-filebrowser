...
from filebrowser.core.functions import get_breadcrumbs, get_filterdate, handle_file_upload, convert_filename
from filebrowser.core.templatetags.fb_tags import query_helper
from filebrowser.core.base import FileListing, FileObject
from filebrowser.core.decorators import path_exists, file_exists
from filebrowser.core.storage import FileSystemStorageMixin, StorageMixin
...
class FileBrowserSite(object):
    ...
    def browse(self, request, template='filebrowser/core/index.html',
        header_template='filebrowser/core/include/tableheader.html',
        item_template='filebrowser/core/include/filelisting.html'):
        """
    ...
    def createdir(self, request, template='filebrowser/core/createdir.html'):
        """
        Create Directory.
        """
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    redirect_url = reverse("%s:fb_browse" % self.app_name, current_app=self.name) + query_helper(query, "ot=desc,o=date", "ot,o,filter_type,filter_date,q,p")
    ...
    def upload(self, request, template='filebrowser/core/upload.html'):
        """
        Multipe File Upload.
        """
        query = request.GET
    ...
    def delete_confirm(self, request, template='filebrowser/core/delete_confirm.html'):
        """
    ...
    def delete(self, request):
        ...
        if request.GET:
            ...
            # XXX: Can't catch only OSError here, since remote storages won't raise that
                ...
                pass
        redirect_url = reverse("%s:fb_browse" % self.app_name, current_app=self.name) + query_helper(query, "", "filename,filetype")
        return HttpResponseRedirect(redirect_url)
    ...
    def detail(self, request, template='filebrowser/core/detail.html'):
        """
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    if "_continue" in request.POST:
                        redirect_url = reverse("%s:fb_detail" % self.app_name, current_app=self.name) + query_helper(query, "filename="+new_name, "filename")
                    else:
                        redirect_url = reverse("%s:fb_browse" % self.app_name, current_app=self.name) + query_helper(query, "", "filename")
                    return HttpResponseRedirect(redirect_url)
                except OSError, (errno, strerror):
    ...
    def _upload_file(self, request):
        ...
        if request.method == "POST":
            ...
            fb_uploadurl_re = re.compile(r'^.*(%s)' % reverse("%s:fb_upload" % self.app_name, current_app=self.name))
...
site = FileBrowserSite(name='filebrowser', app_name='filebrowser.core')
#site = FileBrowserSite(name='filebrowser_core', app_name='filebrowser.core')
