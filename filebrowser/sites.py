...
def get_filterdate(filter_date, date_time):
    ...
    if (
        filter_date == "today"
        and int(date_year) == int(localtime()[0])
        and int(date_month) == int(localtime()[1])
        and int(date_day) == int(localtime()[2])
    ):
        returnvalue = "true"
...
class FileBrowserSite:
    ...
    def get_urls(self):
        ...
        urlpatterns = [
            ...
            re_path(
                r"^delete/$",
                file_exists(self, path_exists(self, filebrowser_view(self.delete))),
                name="fb_delete",
            ),
            re_path(
                r"^detail/$",
                file_exists(self, path_exists(self, filebrowser_view(self.detail))),
                name="fb_detail",
            ),
            re_path(
                r"^version/$",
                file_exists(self, path_exists(self, filebrowser_view(self.version))),
                name="fb_version",
            ),
            re_path(
                r"^upload_file/$",
    ...
    def browse(self, request):
        ...
        for fileobject in listing:
            ...
            append = False
            if (
                (not filter_type or fileobject.filetype == filter_type)
                and (
                    not filter_date or get_filterdate(filter_date, fileobject.date or 0)
                )
                and (not filter_format or filter_format in fileobject.format)
            ):
                append = True
            # search
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    errno = e.args[0]
                    if errno == 13:
                        form.errors["name"] = forms.utils.ErrorList(
                            [_("Permission denied.")]
                        )
                    else:
                        form.errors["name"] = forms.utils.ErrorList(
    ...
    def delete_confirm(self, request):
        ...
        fileobject = FileObject(
            os.path.join(path, query.get("filename", "")), site=self
        )
        if fileobject.filetype == "Folder":
            filelisting = self.filelisting_class(
    ...
    def detail(self, request):
        ...
        if request.method == "POST":
            form = ChangeForm(
                request.POST, path=path, fileobject=fileobject, filebrowser_site=self
            )
            if form.is_valid():
                ...
                try:
                    ...
                    if action_name:
                        ...
                        action_response = action(
                            request=request, fileobjects=[fileobject]
                        )
                        # Post-action signal
                    ...
                    if new_name != fileobject.filename:
                        ...
                        fileobject.delete_versions()
                        self.storage.move(
                            fileobject.path, os.path.join(fileobject.head, new_name)
                        )
                        ...
                        messages.add_message(
                            request, messages.SUCCESS, _("Renaming was successful.")
                        )
                    ...
                    if "_continue" in request.POST:
                        ...
                        ) + query_helper(query, "filename=" + new_name, "filename")
                    else:
                        redirect_url = reverse(
                            "filebrowser:fb_browse", current_app=self.name
                        ) + query_helper(query, "", "filename")
                    return HttpResponseRedirect(redirect_url)
                except OSError:
                    form.errors["name"] = forms.utils.ErrorList([_("Error.")])
        else:
            form = ChangeForm(
                initial={"name": fileobject.filename},
                path=path,
                fileobject=fileobject,
                filebrowser_site=self,
            )
    ...
    def _upload_file(self, request):
        ...
        if request.method == "POST":
            ...
            fb_uploadurl_re = re.compile(
                r"^.*(%s)" % reverse("filebrowser:fb_upload", current_app=self.name)
            )
            ...
            signals.filebrowser_pre_upload.send(
                sender=request, path=folder, file=filedata, site=self
            )
            ...
            signals.filebrowser_post_upload.send(
                sender=request, path=folder, file=f, site=self
            )
